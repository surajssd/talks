...
func main() {
	...
	introContents, err := ioutil.ReadFile("intro_example.yml")
	if err != nil {
		logrus.Fatalln(err)
	}

	introContentsJSON, err := yamlTojson.YAMLToJSON(introContents)
	if err != nil {
		logrus.Fatalln("y2j conversion:", err)
	}

	doc := gojsonschema.NewStringLoader(string(introContentsJSON))
